...
from jax import Array, grad, hessian, jacobian
...
Optimizers: TypeAlias = Literal['RGD', 'PerfGDReparam', 'DPerfGD', 'RRM', 'PerfGDReinforce', 'RegRRM', 'TwoStage', 'DFO']
...
class RGD(Optimizer[Y], Generic[Y]):
    ...
    def step(self, params: Array, x: Array, y: Y) -> Array:
        self.grads = grad(lambda params: jnp.mean(self.loss_fn(params, x, y)))(self.current_params)
        ...
        updates, self.opt_state = self.optimizer.update(self.grads, self.opt_state, params)
...
class RRM(Optimizer[Y], Generic[Y]):
    ...
    def compute_mean(self, params_list):
        ...
        return jax.tree_util.tree_map(
            lambda *arrays: jnp.mean(jnp.stack(arrays), axis=0) if all(isinstance(a, jnp.ndarray) for a in arrays) else
            arrays[0],
            *params_list
        )

    def step(self, params: Array, x: Array, y: Y) -> Array:
        total_diff = jnp.finfo(jnp.float64).max  # initial value for grads so it enters in while loop
        history_grads = []
        j = 0
        while total_diff > self.tol:
            lr = self.lr * (0.9 ** j)

            grads = grad(lambda params: jnp.mean(self.loss_fn(params, x, y)))(self.current_params)
            params_new = jax.tree_util.tree_map(
                lambda x, y: self.proj_fn(x - self.lr * y) if isinstance(x, jnp.ndarray) else x, params, grads)

            diff = jax.tree_util.tree_map(
                lambda x, y: jnp.linalg.norm(x - y) if isinstance(x, jnp.ndarray) else x, params_new, params)
            total_diff = sum(jnp.sum(leaf) for leaf in jax.tree_util.tree_leaves(diff) if isinstance(leaf, jnp.ndarray))
    ...
    def step(self, params: Array, x: Array, y: Y) -> Array:
        ...
        while total_diff > self.tol:
            grads = grad(
                ...
                + self.reg * jnp.linalg.norm(params - self.params_history[-1] + 1e-8) ** 2
            )(self.current_params)
...
class PerfGDReinforce(Optimizer[Y], Generic[Y]):
    ...
    def delta_f_theta(self):
        ...
        delta_theta = (
            jnp.array(self.params_history[self.i - self.H : self.i]) - self.params_history[self.i]
        ).T
        delta_f = (jnp.array(self.f_history[self.i - self.H : self.i]) - self.f_history[self.i]).T
    ...
    def _grad2(self, params: Array, x: Array, y: Y) -> Array:
        ...
        jacobians = jacobian(lambda mean: jnp.squeeze(self.prob_distr(x, y, mean, params)))(
    ...
    def step(self, params: Array, x: Array, y: Y) -> Array:
        ...
        if self.i < self.H:
            ...
            grad1 = grad(lambda params: jnp.mean(self.loss_fn(params, x, y)))(self.current_params)

            grad2 = self._grad2(params, x, y)
            grads = jax.tree_util.tree_map(
                lambda g1, g2: g1 + g2 if isinstance(g1, jnp.ndarray) and isinstance(g2, jnp.ndarray) else g1,
                grad1, grad2
            )
        self.grads = grads
        # self.current_params = jnp.squeeze(self.proj_fn(params - self.lr * grads))

        self.current_params = jax.tree_util.tree_map(lambda x, y: self.proj_fn(x - self.lr * y) if isinstance(x, jnp.ndarray) else x, params, grads)
...
class PerfGDReparam(Optimizer[Y], Generic[Y]):  # Especial Gradient Descent
    ...
    def step(self, params: Array, x: Array, y: Y) -> Array:
        ...
        updates, self.opt_state = self.optimizer.update(self.grads, self.opt_state, params)
        self.current_params = optax.apply_updates(params, updates)
...
class DPerfGD(Optimizer[Y], Generic[Y]):  # Decoupled Gradient Descent
    ...
    def step(self, params: Array, x: Array, y: Y) -> Array:
        ...
        def decoupled_loss(p_m: Array, p_d: Array) -> Array:
            ...
            return jnp.mean(self.loss_fn(p_m, x=x_0, y=y_0)) #+ self.reg * jnp.sum(jnp.abs(p_m - p_d + 1e-8))
        ...
        updates_M, self.opt_state_M = self.optimizer_M.update(grad_M, self.opt_state_M, params)
        self.current_params = optax.apply_updates(params, updates_M)

        updates_D, self.opt_state_D = self.optimizer_D.update(grad_D, self.opt_state_D, self.current_p_d)
        self.current_p_d = optax.apply_updates(self.current_p_d, updates_D)
...
class DFO(Optimizer[Y], Generic[Y]):
    ...
    def step(self, params, x: Array, y: Y) -> Array:
        def sample_unit_sphere(dim, num_samples, seed):
            """
            Generate samples uniformly on the unit sphere S^{d-1}.
            """
            samples = jax.random.normal(jax.random.PRNGKey(seed), shape=(num_samples, *dim))
            samples /= jnp.linalg.norm(samples, axis=tuple(range(1, len(samples.shape))), keepdims=True)
            return samples

        def decoupled_loss(p_p, p):
            x, y = self.distr_shift(p_p)
            return jnp.mean(self.loss_fn(p, x, y))

        def performative_risk(params):
            return decoupled_loss(params, params)


        u_samples = jax.tree_util.tree_map(lambda params: sample_unit_sphere(params.shape, self.samples, self.seed), params)

        perturbed_params = jax.tree_map(lambda u_samples, params: params + self.delta * u_samples, u_samples, params)
        risks = jax.vmap(performative_risk)(perturbed_params)

        grads = jax.tree_util.tree_map(lambda u_samples: jnp.mean(risks.reshape((self.samples, ) + (1, ) * (u_samples.ndim - 1)) * u_samples, axis=0), u_samples)

        # Update parameters using the computed gradients
        updated_params = jax.tree_util.tree_map(
            lambda p, g: self.proj_fn(p - self.lr * g) if isinstance(p, jnp.ndarray) else p,
            params,
            grads
        )
