...
from examples.utils import initialize_params, acc_fn, weight_norm
...
class Credit:
    ...
    def init_model(self):
        if self.model == 'NN':
            layers = [layer for _ in range(1) for layer in [
    ...
    def train(
    ...
    ) -> tuple[Optimizer, list, list]:

        start_time = time.time()

        logger = Logger(
            project="decoupled-loss", group=f"{self.output_file}", name=optimizer_name + f'_model_{self.model}_{self.base_optimizer}_M{self.momentum}_S{self.rho}_{self.seed}',
            config=asdict(self), upload=self.log_wandb
        )
        ...
        try:
            ...
            match optimizer_name:
                ...
                case 'RRM':
                    optimizer = RRM(
                        params, lr=self.lr, loss_fn=self.loss_fn, proj_fn=self.proj_fn, tol=0.001
                    )
                ...
                for i in range(self.iterations):
                    ...
                    pbar.set_description(
                        "Performative_loss: {0:.4f} Accuracy: {1:.2f}%".format(
                            current_loss.item(), self.accuracy(params, x, y).item() * 100
                        )
                    )
