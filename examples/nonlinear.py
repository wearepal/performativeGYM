...
class NonLinear:
    ...
    def shift_data_distribution(
        ...
        return jnp.expand_dims(jnp.sqrt(self.A1 * params + self.A0) + z * self.STD, axis=1), None
    ...
    def init_model(self):
        return 0.85 + initialize_params((1,), self.seed) * 0.1 #changes the std of initialization
    ...
    def log_decoupled_landscape(self):
        logger = Logger(
            project="decoupled-loss", group="landscape", name='nonlinear', config=asdict(self), upload=self.log_wandb
        )
    ...
    def train(
        ...
        logger = Logger(
            project="PerfGD", group="nonlinear", name=f'{optimizer_name}_{self.seed}', config=asdict(self), upload=self.log_wandb
        )
...
if __name__ == '__main__':
    args = tyro.cli(NonLinear, use_underscores=True)
    start_time = time.time()
