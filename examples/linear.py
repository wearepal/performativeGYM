...
class Linear:
    ...
    def shift_data_distribution(
        ...
        return jnp.expand_dims((self.A1 * params + self.A0) + z * self.STD, axis=1), None
    ...
    def log_decoupled_landscape(self):
        logger = Logger(
            project="decoupled-loss", group="landscape", name='linear', config=asdict(self), upload=self.log_wandb
        )
